...
target_x <- 0.02
target_y <- predict(mod, newdata = data.frame(hepi_rate = target_x)) - 0.005
...
ggsave("SSI_HEPI_GAP.svg", width = 8, height = 6)

#Next plot series with Okabe-Ito palette
# Okabe-Ito palette
okabe_ito <- c(
  "blue" = "#0072B2",
  "orange" = "#E69F00",
...
inflation_df <- final_df %>%
  select(fiscal_year, hepi_rate, cpi_rate) %>%
  pivot_longer(-fiscal_year, names_to = "series", values_to = "rate") %>%
  mutate(
    series = recode(series, hepi_rate = "HEPI", cpi_rate = "CPI")
  )

label_data <- inflation_df %>%
  filter(fiscal_year == max(fiscal_year))

ggplot(inflation_df, aes(x = fiscal_year, y = rate, color = series)) +
  annotate(
    "rect",
    xmin = 2021.5,
    xmax = 2023.5,
  ...
  geom_point(size = 2) +
  scale_color_manual(values = c("HEPI" = "#D55E00", "CPI" = "#CC79A7")) +
  scale_y_continuous(labels = percent_format()) +
  scale_x_continuous(
    breaks = seq(min(final_df$fiscal_year), max(final_df$fiscal_year), 1)
  ) +
  labs(
    title = "<span style='color:#D55E00;'>HEPI</span> Outpaces Inflation<br>Except During 2022 Price Spike",
    x = "Fiscal Year",
    y = "Percent Change",
    color = NULL
  ) +
...
ggsave("HEPI_V_CPI.svg")
ggsave("HEPI_V_CPI2.svg", width = 8, height = 6)


ggplot(final_df, aes(x = factor(fiscal_year), y = total_fte)) +
  geom_col(fill = "#009E73") +
  scale_y_continuous(labels = comma_format()) +
  labs(
    title = "Enrollment Has Declined Since Its Peak",
    subtitle = "Peak in 2016, Lowest in 2023",
    x = "Fiscal Year",
    y = "Total Enrollment"
  ) +
  theme_minimal()
